...
void MainWindow::resetKeyStates()
{
    aPressed = dPressed = false;
    leftPressed = rightPressed = false;
    wPressed = upPressed = false;
    qPressed = kPressed = false;
    gameWorld.setInput(false, false, false, false, false, false, false, false);
}

void MainWindow::initAudio()
{
    fartSound = new QSoundEffect(this);
    fartSound->setSource(QUrl::fromLocalFile("res/sounds/fart.wav"));
    fartSound->setVolume(0.8f);

    attackSound = new QSoundEffect(this);
    attackSound->setSource(QUrl::fromLocalFile("res/sounds/attack.wav"));
    attackSound->setVolume(0.7f);

    bgmPlayer = new QMediaPlayer(this);
    bgmOutput = new QAudioOutput(this);
    bgmPlayer->setAudioOutput(bgmOutput);
    bgmOutput->setVolume(0.4f);
}

void MainWindow::playBGMForState(int forceLevel)
{
    QString bgmFile;
    int level = forceLevel;
    if (level == -1) {
        if (gameState == MENU || gameState == GUIDE || gameState == LEVEL_SELECT) {
            bgmFile = "res/sounds/bgm_menu.wav";
        } else if (gameState == PLAYING) {
            level = gameWorld.getCurrentLevel();
            if (level == 1) bgmFile = "res/sounds/bgm_level1.wav";
            else if (level == 2) bgmFile = "res/sounds/bgm_level2.wav";
            else if (level == 3) bgmFile = "res/sounds/bgm_level3.wav";
            else return;
        } else {
    ...
    if (bgmPlayer->source() != QUrl::fromLocalFile(bgmFile)) {
        bgmPlayer->setSource(QUrl::fromLocalFile(bgmFile));
        bgmPlayer->play();
    } else if (bgmPlayer->playbackState() != QMediaPlayer::PlayingState) {
        bgmPlayer->play();
    }
}
...
{
    int current = gameWorld.getCurrentLevel();
    int next = current + 1;
    if (next <= 3) {
        showPrelude(next);
    } else {
...
void MainWindow::updateGame()
{
    if (gameState == PLAYING) {
        ...
        if (p1Jump && fartTimer1 == 0) {
            ...
            if (fartSound && fartSound->isLoaded()) fartSound->play();
        }
        ...
        static int lastAttack1 = 0, lastAttack2 = 0;
        int curAttack1 = gameWorld.getAttackEffectTimer1();
        int curAttack2 = gameWorld.getAttackEffectTimer2();
        if (curAttack1 > 0 && lastAttack1 == 0 && attackSound && attackSound->isLoaded())
            attackSound->play();
        if (curAttack2 > 0 && lastAttack2 == 0 && attackSound && attackSound->isLoaded())
            attackSound->play();
        lastAttack1 = curAttack1;
        lastAttack2 = curAttack2;
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    case GUIDE:
        ...
        painter.drawText(100, 460, "小心路上的障碍物和攻击者");
        break;
    case LEVEL_SELECT:
    ...
    case PRELUDE: {
        ...
        if (pendingLevel == 1) {
            ...
            prelude = preludePic2;
            title = "童年回忆2";
            content = "图图、小美和刷子逃课一起出去玩，图图妈发现之后非常生气，想要好好收拾图图一顿，快去解救他们！";
        } else {
            prelude = preludePic3;
            title = "童年回忆3";
        ...
        int titleWidth = fmTitle.horizontalAdvance(title) + 60;
        QRect titleRect((800 - titleWidth) / 2, 100, titleWidth, 70);
        ...
        QRect contentRect(100, 200, 600, 220);
        painter.setBrush(QColor(255, 255, 255, 220));
        painter.setPen(Qt::NoPen);
        painter.drawRoundedRect(contentRect, 15, 15);
        painter.setPen(Qt::black);
        painter.setFont(QFont("微软雅黑", 18));
        painter.drawText(contentRect, Qt::TextWordWrap | Qt::AlignCenter, content);
        break;
    }
    case PLAYING:
        drawGame(painter);
    ...
    case VICTORY:
        ...
        painter.setPen(Qt::yellow);
        painter.setFont(QFont("微软雅黑", 20));
        QString victoryText = "你已经成功拾取所有童年回忆！";
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_K: kPressed = true; break;
    default: return;
    }
    gameWorld.setInput(aPressed, dPressed, leftPressed, rightPressed, wPressed, upPressed, qPressed, kPressed);
}
...
void MainWindow::keyReleaseEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_K: kPressed = false; break;
    default: return;
    }
    gameWorld.setInput(aPressed, dPressed, leftPressed, rightPressed, wPressed, upPressed, qPressed, kPressed);
}